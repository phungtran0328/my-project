            ...
            <div class="col-md-5">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <form class="" action="{{url('/admin/author', $author->TG_MA)}}" method="post">
                            <input type="hidden" name="_token" value="{{csrf_token()}}">
                            @method('PATCH')
                            <fieldset>
                                <div class="form-group {{$errors->has('name') ? 'has-error' : ''}}">
                                    <label class="control-label">Tên tác giả</label>
                                    <input class="form-control" value="{{$author->TG_TEN}}" name="name">
                                    <strong style="color: red">{{$errors->first('name')}}</strong>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Mô tả</label>
                                    <input type="text" class="form-control" value="{{$author->TG_MOTA}}" name="description">
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Ghi chú</label>
                                    <input type="text" class="form-control" value="{{$author->TG_GHICHU}}" name="note">
                                </div>
                                <div class="form-group">
